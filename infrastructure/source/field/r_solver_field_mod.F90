...
!-----------------------------------------------------------------------------
!
!> @brief A module providing r_solver field related classes.
!>
!> @details This is a version of a field object that can hold r_solver data
!> values. It contains both a representation of an r_solver field which provides
...
!______r_solver_field_type_____________________________________________________

  !> Algorithm layer representation of an r_solver field.
  !>
  ...
  !> alone.
  !>
  type, extends(field_parent_type), public :: r_solver_field_type
    private

    !> The r_solver values of the field
    real(kind=r_solver), allocatable :: data( : )
  ...
  end type r_solver_field_type

!______r_solver_field_pointer_type_______________________________________________________

!> a class to hold a pointer to an r_solver field in an object that is a child
!> of the pure abstract field class

  type, extends(pure_abstract_field_type), public :: r_solver_field_pointer_type
    !> A pointer to an r_solver field
    type(r_solver_field_type), pointer, public :: field_ptr
  contains
    !> Initialiser for a field. May only be called once.
    procedure, public :: r_solver_field_pointer_initialiser
    !> Finaliser for a field pointer object
    final :: r_solver_field_pointer_destructor
  end type r_solver_field_pointer_type
...
!______field_proxy_type_______________________________________________________
  ...
  !>
  type, extends(field_parent_proxy_type), public :: r_solver_field_proxy_type

    private

    !> Allocatable array of type r_solver which holds the values of the field
    real(kind=r_solver), public, pointer :: data( : ) => null()
  ...
  end type r_solver_field_proxy_type

!______end of type declarations_______________________________________________

contains

!______r_solver_field_type_procedures__________________________________________
  ...
  !> field_type
  type(r_solver_field_proxy_type) function get_proxy(self)
    implicit none
    class(r_solver_field_type), target, intent(in)  :: self
  ...
  subroutine field_initialiser(self, &
    ...
    class(r_solver_field_type), intent(inout)        :: self
  ...
  !> Initialise an r_solver field pointer
  !>
  !> @param [in] field_ptr A pointer to the r_solver field that is to be
  !>                       stored as a reference
  subroutine r_solver_field_pointer_initialiser(self, field_ptr)
    implicit none
    class(r_solver_field_pointer_type) :: self
    type(r_solver_field_type), pointer :: field_ptr
    self%field_ptr => field_ptr
  end subroutine r_solver_field_pointer_initialiser

  ! Finaliser for an r_solver field pointer
  !
  ...
  subroutine r_solver_field_pointer_destructor(self)
    implicit none
    type(r_solver_field_pointer_type), intent(inout) :: self
  end subroutine r_solver_field_pointer_destructor
  ...
  subroutine copy_field(self, dest, name)
    ...
    implicit none
    class(r_solver_field_type), target, intent(in)  :: self
    class(r_solver_field_type), target, intent(out) :: dest
    character(*), optional, intent(in)              :: name
  ...
  subroutine field_type_assign(dest, source)
    ...
    use log_mod,         only : log_event, &
                                ...
                                LOG_LEVEL_INFO
    implicit none
    class(r_solver_field_type), intent(in)  :: source
    class(r_solver_field_type), intent(out) :: dest
  ...
  subroutine field_destructor_scalar(self)
    ...
    type(r_solver_field_type), intent(inout)    :: self
  ...
  subroutine get_write_behaviour(self, write_behaviour)
    ...
    class(r_solver_field_type), intent(in)             :: self
  ...
  subroutine log_minmax( self, log_level, label )
    ...
    character( * ),               intent(in)         :: label
    integer(i_def)                                   :: i
    real(r_solver)                                   :: l_min, l_max
    real(r_solver)                                   :: answer_min, answer_max
  ...
  subroutine write_field(this, field_name)
    ...
    class(r_solver_field_type),   intent(in)     :: this
  ...
  subroutine read_field( self, field_name)
    ...
    class( r_solver_field_type ),  target, intent( inout ) :: self
    character(len=*),              intent(in)              :: field_name


    type( r_solver_field_proxy_type )                      :: tmp_proxy
  ...
  subroutine halo_exchange( self, depth )
    ...
    class( r_solver_field_proxy_type ), target, intent(inout) :: self
  ...
  function get_sum(self) result (answer)
    ...
    class(r_solver_field_proxy_type), intent(in) :: self
