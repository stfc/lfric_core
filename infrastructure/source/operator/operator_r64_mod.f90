...
!-----------------------------------------------------------------------------
!
!> @brief A module providing 64-bit operator related classes.
!>
!> @details Implements the locally assembled 64-bit operator (i.e. the stencil
!>          is assembled in each cell of the 3d grid)

module operator_r64_mod
  ...
  !>
   type, public, extends(operator_parent_type) :: operator_r64_type
    private
    !> Allocatable array of type real which holds the values of the operator
    real(kind=real64), allocatable :: local_stencil( :, :, : )
    !> Size of the outermost dimemsion of the local_stencil array, equal to
    !! ncell*nlayers
    integer(i_def) :: ncell_3d
  contains
    !> Initialise an operator object
    procedure, public :: initialise => operator_r64_initialiser
    !> Function to get a proxy with public pointers to the data in a
    !! operator_r64_type.
    procedure, public :: get_proxy => get_proxy_operator_r64
    !> Deep copy methods
    procedure, private :: operator_r64_type_deep_copy
    procedure, public  :: deep_copy => operator_r64_type_deep_copy
 ...
 !>
 type, public, extends(operator_parent_proxy_type) :: operator_r64_proxy_type
    private
    !> Allocatable array of type real which holds the values of the operator
    real(kind=real64), public, pointer :: local_stencil( :, :, : )
    !> size of the outermost dimension
    integer(i_def), public :: ncell_3d
    integer(i_def), allocatable :: gnu_dummy(:)
  contains
    final :: operator_r64_proxy_destructor
 end type operator_r64_proxy_type

!______end of type declarations_______________________________________________

contains

  !> Initialise an <code>operator_r64_type</code> object.
  !>
  ...
  !>
  subroutine operator_r64_initialiser( self, fs_to, fs_from )
    ...
    implicit none
    class(operator_r64_type),   target, intent(inout) :: self
  ...
  !> Function to create a proxy with access to the data in the operator_r64_type.
  !>
  !> @return The proxy type with public pointers to the elements of
  !> operator_r64_type
  type(operator_r64_proxy_type ) function get_proxy_operator_r64(self)
    implicit none
    class(operator_r64_type), target, intent(in)  :: self

    ! Call the routine that initialises the proxy for data held in the parent
    call self%operator_parent_proxy_initialiser(get_proxy_operator_r64)

    get_proxy_operator_r64 % local_stencil           => self % local_stencil
    get_proxy_operator_r64 % ncell_3d                =  self%ncell_3d
  end function get_proxy_operator_r64
  ...
  end function operator_r64_type_deep_copy
  ...
  subroutine operator_r64_destructor(self)
    implicit none
    type(operator_r64_type), intent(inout) :: self
    call self%destroy_operator_parent()
    call self%operator_final()
  end subroutine operator_r64_destructor

  !>@brief Destroy the operator proxy
  subroutine operator_r64_proxy_destructor(self)
    implicit none
    type(operator_r64_proxy_type) :: self
    call self%destroy_operator_parent_proxy()
    nullify(self%local_stencil)
  end subroutine operator_r64_proxy_destructor

end module operator_r64_mod
