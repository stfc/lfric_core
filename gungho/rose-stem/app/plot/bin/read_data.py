...
def read_nodal_data(filestem, ncomp, comp):
  ...
  if ncomp == 3: # vector fields with 3 components
    ...
    col_types = {'x': np.float32, 'y': np.float32, 'z' : np.float32, 'level' : np.float32,'c1': np.float64, 'c2': np.float64, 'c3' : np.float64 }
  else:         # scalar fields with 1 component
    col_names = ['x', 'y', 'z', 'level', 'c1']
    col_types = {'x': np.float32, 'y': np.float32, 'z' : np.float32, 'level' : np.float32,'c1': np.float64}
  ...
  if len(dirlist) < 1:
    print("Warning: No files found to plot")
  else:
  
    for f in dirlist:
 
      tmp_data = pd.read_csv(f, header=None, names=col_names, skiprows=1, comment=']', dtype=col_types, sep='\s+')
    ...
    if ncomp == 3: # vector field
      if comp != '3': # u or v component
        all_data = all_data.loc[all_data['level'].mod(1) == 0.5] # Half levels 
      else: # w component
        all_data = all_data.loc[all_data['level'].mod(1) == 0.0] # Full levels
