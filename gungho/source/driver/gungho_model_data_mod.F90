...
contains
  ...
  !> Adds a field_collection to the model data. The field maintained in the
  !> collection will either be a copy of the original or a field pointer object
  !> containing a pointer to a field held elsewhere.
  ...
  !> Check if a field collection is present the model data
  ...
  !> Generates a field_collection and adds it to the model data.
  ...
  !> Access a field collection from the collection
  !> @param [in] field_collection_name The name of the field collection to be &
  !>                                   retrieved
  ...
  subroutine finalise_model_data( model_data )
    ...
    implicit none
      ...
      call model_data%field_collection_list%clear()
