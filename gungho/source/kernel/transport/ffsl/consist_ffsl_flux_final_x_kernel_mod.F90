...
!-----------------------------------------------------------------------------
!> @brief Calculates the final flux in x for FFSL for consistent tracers.
...
module consist_ffsl_flux_final_x_kernel_mod
  ...
  type, public, extends(kernel_type) :: consist_ffsl_flux_final_x_kernel_type
    private
  ...
  contains
    procedure, nopass :: consist_ffsl_flux_final_x_code
  end type

  !-------------------------------------------------------------------------------
  ! Contained functions/subroutines
  !-------------------------------------------------------------------------------
  public :: consist_ffsl_flux_final_x_code

contains

  !> @brief Compute the x-flux for the final horizontal stage of consistent FFSL
  !> @param[in]     nlayers           Number of layers
  !> @param[in,out] flux_high         The output high order fractional flux in x
  !> @param[in,out] flux_low          The output low order fractional flux in x
  !> @param[in,out] flux_int          The output integer flux in x
  ...
  !> @param[in]     edges_spt         Logical for using (not) special edges treatment
  ...
  subroutine consist_ffsl_flux_final_x_code( nlayers,          &
                                             flux_high,        &
                                             flux_low,         &
                                             flux_int,         &
                                             field_x,          &
    ...
    real(kind=r_tran)   :: rho_d_local(1:stencil_size_y)
        ...
        if ( flux_high(map_w2h(local_dofs(dof_iterator)) ) == 0.0_r_tran .AND. &
          ...
          do k = 0,nlayers-1
            ...
            do ijk = i_start(map_wp(1)), i_end(map_wp(1))
              field_local(ijk) = field_x_local(ijk)
              rho_d_local(ijk) = rho_d_x_local(ijk)
            end do
  ...
  end subroutine consist_ffsl_flux_final_x_code

end module consist_ffsl_flux_final_x_kernel_mod
