...
!-----------------------------------------------------------------------------
!> @brief Calculates the first flux in y for consistent FFSL.
...
module consist_ffsl_flux_first_y_kernel_mod
  ...
  use argument_mod,       only : arg_type,                  &
                                 ...
                                 GH_LOGICAL
  use constants_mod,      only : i_def, r_tran, l_def, r_def, r_def
  ...
  type, public, extends(kernel_type) :: consist_ffsl_flux_first_y_kernel_type
    private
    type(arg_type) :: meta_args(14) = (/                                                     &
         ...
         arg_type(GH_FIELD,  GH_REAL,    GH_READ,  W3, STENCIL(Y1D)),                        & ! rho_d
  ...
  contains
    procedure, nopass :: consist_ffsl_flux_first_y_code
  end type

  !-------------------------------------------------------------------------------
  ! Contained functions/subroutines
  !-------------------------------------------------------------------------------
  public :: consist_ffsl_flux_first_y_code

contains

  !> @brief Compute the first fluxes in y for consistent FFSL.
  ...
  subroutine consist_ffsl_flux_first_y_code( nlayers,          &
                                             flux_high,        &
                                             flux_low,         &
                                             flux_int,         &
                                             field,            &
        ...
        if ( flux_high(map_w2h(local_dofs(dof_iterator)) ) == 0.0_r_tran .AND. &
          ...
          do k = 0,nlayers-1
            ...
            do jj = 1, stencil_half
              ...
              rho_d_local(jj) = rho_d(stencil_map_rho(1,stencil_half+1-jj) + k)
              detj_local(jj) = detj(stencil_map_dj(1,stencil_half+1-jj) + k)
            end do
            do jj = stencil_half+1, stencil_size
              field_local(jj) = field(stencil_map(1,jj) + k)
              rho_d_local(jj) = rho_d(stencil_map_rho(1,jj) + k)
  ...
  end subroutine consist_ffsl_flux_first_y_code

end module consist_ffsl_flux_first_y_kernel_mod
