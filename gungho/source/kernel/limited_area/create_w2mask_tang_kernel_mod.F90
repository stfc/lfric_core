...
!-----------------------------------------------------------------------------
!> @brief Computes the mask to define the tangential winds on the boundary.
!> @details The mask is defined using the W2 dofs. The coordinates of the dofs
!!          are compared with the coordinates of the faces of the cells 1
!!          cell in from the boundary, to within a prescribed tolerance.
!!          The tangential winds are used to define the boundary conditions
...
!> @param[in,out] u_mask   The mask to define the tangential winds on boundary
...
subroutine create_w2mask_tang_code( nlayers,      &
                                    boundary_s,   &
                                    boundary_n,   &
                                    boundary_e,   &
                                    boundary_w,   &
  ...
  do df1 = 1, ndf_w2
    ...
    ! Define the mask by querying whether the dof is in the inner box
    ! and the outerbox.
    ...
    ! 000|1--innerbox--1|000
    ! 000|1|0000000000|1|000
    ! 000|1|0000000000|1|000
    ! 000|1------------1|000
    ! 000|11111111111111|000
    ! 000----------------000
    ...
    ! We set the dofs on both the inner and outer box boundaries to 0.

    ! Is the dof in (but not on, hence > ) the Outerbox boundary?
    ...
    ! Is the dof in ( and on, hence >= ) the Innerbox boundary?
    ...
    ! Step 2. Set all the dofs outside (but not on) the innerbox to 1.
    ...
    ! Step 3. Set all the dofs outside (and on) the outerbox to 0.
    if ( rim_width_ns >= 0_i_def ) then
...
end subroutine create_w2mask_tang_code

end module create_w2mask_tang_kernel_mod
