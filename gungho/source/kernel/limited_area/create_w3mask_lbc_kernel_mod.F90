...
!!        region for the W3 function space.
!> @details The coordinates of the W3 dofs are compared with the coordinates
!!          of the boundary. The mask takes the value 1 between the inner and
!!          outer boundaries and 0 otherwise. This mask is used to overwrite
!!          data in the LBC region by the boundary condition data from the
!!          driving model.
module create_w3mask_lbc_kernel_mod
  ...
  type, public, extends(kernel_type) :: create_w3mask_lbc_kernel_type
    private
  ...
  contains
    procedure, nopass :: create_w3mask_lbc_code
  end type

  !-------------------------------------------------------------------------
  ! Contained functions/subroutines
  !-------------------------------------------------------------------------
  public :: create_w3mask_lbc_code
...
!> @param[in,out] rho_mask The mask to define the LBCs for W3 (density, rho)
...
subroutine create_w3mask_lbc_code( nlayers,    &
                                   boundary_s, &
                                   boundary_n, &
                                   boundary_e, &
                                   boundary_w, &
  ...
  real(kind=r_def),                              intent(in) :: boundary_s,     &
                                                               ...
                                                               rim_e,          &
                                                               rim_w
  real(kind=r_def), dimension(undf_w3),          intent(inout) :: rho_mask
  ...
  do df1 = 1, ndf_w3
    ...
    do k = 0, nlayers-1
      rho_mask(map_w3(df1) + k) = mask_value
    enddo

  end do

end subroutine create_w3mask_lbc_code

end module create_w3mask_lbc_kernel_mod
