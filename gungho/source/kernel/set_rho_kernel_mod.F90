...
subroutine set_rho_code(nlayers, rho,                           &
  ...
  integer(kind=i_def), intent(in) :: ndf_rho, ndf_chi_sph, ndf_pid
  integer(kind=i_def), intent(in) :: undf_rho, undf_chi_sph, undf_pid
  ...
  do k = 0, nlayers-1
    do df1 = 1, ndf_chi_sph
      chi_sph_1_e(df1) = chi_sph_1( map_chi_sph(df1) + k )
      chi_sph_2_e(df1) = chi_sph_2( map_chi_sph(df1) + k )
      chi_sph_3_e(df1) = chi_sph_3( map_chi_sph(df1) + k )
    end do
    call coordinate_jacobian(ndf_chi_sph, nqp_h, nqp_v,                 &
                             chi_sph_1_e, chi_sph_2_e, chi_sph_3_e,     &
                             ipanel, chi_sph_basis, chi_sph_diff_basis, &
                             jac, dj)
      ...
      do qp2 = 1, nqp_v
        do qp1 = 1, nqp_h
          ...
          do df2 = 1, ndf_chi_sph
            ...
            coords(2) = coords(2) + chi_sph_2_e(df2)*chi_sph_basis(1,df2,qp1,qp2)
