...
!>        nonlinear equations.
!>
!> @details The kernel computes the boundary integral of the pressure gradient
!>          term for the nonlinear equations, written in the vector invariant
!>          form.
!>          This consists of ru_bd = -cp*theta*v*normal_vector*average(pi),
!>          where average(pi) needs to be considered as exner is discontinuous
      ...
      do face = 1, nfaces_re_h
        ...
        do qp = 1, nqp_f
          ...
          do df = 1, ndf_w3
            ...
            exner_next_at_fquad  = exner_next_at_fquad + &
          ...
          do df = 1, ndf_w2
            ...
            bdary_term = - cp * dot_product(v, outward_normals_to_horizontal_faces(:, face)) * &
                         theta_at_fquad * av_pi_at_fquad
            ru_bd_e(df) = ru_bd_e(df) + wqp_f(qp,face) * bdary_term
          end do
