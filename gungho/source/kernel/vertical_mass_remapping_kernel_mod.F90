...
contains
  ...
  real(kind=r_def), dimension(undf_w2), intent(in)        :: u_adv
  ...
  real(kind=r_def), allocatable :: wl(:)
  ...
  if ( ext > 0 ) then
    do k=1-ext, 1
      wl(k) = real((k+ext-1)/(ext+1),r_def)*wl(2)
    end do
    do k=nzl, nzl+ext
      wl(k) = real((nzl+ext-k)/(ext+1),r_def)*wl(nzl-1)
    end do
    do k=1-ext, 0
      m0(k) = m0(1)
    end do
  ...
  deallocate(zl,zd,wl,dz,m0,mn,a0,a1,a2,a3)
