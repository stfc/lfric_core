...
subroutine apply_helmholtz_operator_code(nlayers, &
  ...
  integer(kind=i_def), intent(in) :: nlayers, smap_size

  integer(kind=i_def), dimension(ndf),           intent(in) :: map
  integer(kind=i_def), dimension(ndf,smap_size), intent(in) :: smap
  ...
  do k = 0,nlayers-1
    y(map(1)+k) = Helm_C(map(1)+k)*x(smap(1,1)+k) &
                + Helm_W(map(1)+k)*x(smap(1,2)+k) &
                + Helm_S(map(1)+k)*x(smap(1,3)+k) &
                + Helm_E(map(1)+k)*x(smap(1,4)+k) &
                + Helm_N(map(1)+k)*x(smap(1,5)+k)
  end do
