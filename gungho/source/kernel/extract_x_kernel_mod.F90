...
!! @param[out] x_field           Extracted x-direction component of input W2 field
...
subroutine extract_x_code( nlayers,                      &
                           cell_orientation,             &
                           w2_field_in,                  &
                           x_field,                      &
                           undf_w3,                      &
  ...
  if (orientation_of_cell > 0_i_def .and. orientation_of_cell < 5_i_def ) then
    dof_vector = (/ w2_dof(orientation_of_cell,1),     &
                    w2_dof(orientation_of_cell,2),     &
                    w2_dof(orientation_of_cell,3),     &
                    w2_dof(orientation_of_cell,4)      /)

    do k=0, nlayers-1

      x_field(map_w2(dof_vector(1))+k) = w2_field_in(map_w2(dof_vector(1))+k)
...
end subroutine extract_x_code

end module extract_x_kernel_mod
