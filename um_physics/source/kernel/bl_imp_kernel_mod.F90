...
module bl_imp_kernel_mod
  ...
  type, public, extends(kernel_type) :: bl_imp_kernel_type
    ...
    type(arg_type) :: meta_args(77) = (/                &
        ...
        arg_type(GH_FIELD,   GH_INC,       ANY_SPACE_2),&! tile_moisture_flux
        ...
        arg_type(GH_FIELD,   GH_READ,      W2),         &! rhokm_surf_w2
        ...
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! alpha1_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! ashtf_prime_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! dtstar_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! fraca_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! z0h_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! z0m_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! rhokh_tile
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_2),&! chr1p5m_tile
        ...
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_1),&! soil_moist_avail
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_1),&! zh_nonloc
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_1),&! zh_2d
        arg_type(GH_FIELD,   GH_READ,      ANY_SPACE_7) &! bl_types
        /)
...
contains
  ...
  subroutine bl_imp_code(nlayers,                            &
    ...
    use init_jules_alg_mod, only:                             &
         n_land_tile, n_sea_ice_tile, n_soil_levs,            &
    ...
    integer(kind=i_def), intent(in) :: ndf_2d, undf_2d, undf_w2
    ...
    real(r_um), dimension(row_length,rows,bl_levels) :: taux, tauy,          &
         f_ngstress_u, f_ngstress_v, dtrdz_u, dtrdz_v, rhokm_u, rhokm_v
    ...
    do i = first_land_tile, first_land_tile + n_land_tile - 1
      ...
      ! sensible heat flux
      ftl_surft(1, i_tile) = real(tile_heat_flux(map_tile(i)), r_um)
      ! moisture flux
      fqw_surft(1, i_tile) = real(tile_moisture_flux(map_tile(i)), r_um)
    end do
    ...
    do i = first_land_tile, first_land_tile + n_land_tile - 1
      i_tile = i_tile + 1
      alpha1(1, i_tile) = alpha1_tile(map_tile(i))
      ashtf_prime_surft(1, i_tile) = ashtf_prime_tile(map_tile(i))
      dtstar_surft(1, i_tile) = dtstar_tile(map_tile(i))
      fraca(1, i_tile) = fraca_tile(map_tile(i))
      z0h_surft(1, i_tile) = z0h_tile(map_tile(i))
      z0m_surft(1, i_tile) = z0m_tile(map_tile(i))
      rhokh_surft(1, i_tile) = rhokh_tile(map_tile(i))
      chr1p5m(1, i_tile) = chr1p5m_tile(map_tile(i))
      resfs(1, i_tile) = resfs_tile(map_tile(i))
      canhc_surft(1, i_tile) = canhc_tile(map_tile(i))
    end do
    ...
    do i = 1, n_land_tile
      do n = 1, n_soil_levs
        i_tile = i_tile + 1
        wt_ext_surft(1,n,i) = tile_water_extract(map_smtile(i_tile))
      end do
    end do

    alpha1_sea(1,1) = alpha1_tile(map_tile(first_sea_tile))
    ashtf_prime_sea(1,1) = ashtf_prime_tile(map_tile(first_sea_tile))
    dtstar_sea(1,1) = dtstar_tile(map_tile(first_sea_tile))
    rhokh_sea(1,1) = rhokh_tile(map_tile(first_sea_tile))
    ...
    bl_type_3(1,1) = bl_types(map_bl(3))
    bl_type_4(1,1) = bl_types(map_bl(4))
    bl_type_5(1,1) = bl_types(map_bl(5))
