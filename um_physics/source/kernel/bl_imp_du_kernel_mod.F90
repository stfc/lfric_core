...
module bl_imp_du_kernel_mod
  ...
  real(kind=r_def), parameter :: sqrt2 = sqrt(2.0_r_def)
...
contains
  ...
  subroutine bl_imp_du_code(nlayers,       &
    ...
    do df = 1,4
      ...
      if (du_bl(map_w2(df)) == 0.0_r_def) then
        ...
        i1 = (1.0_r_def + 1.0_r_def/sqrt2)*(1.0_r_def + pnonl)
        e1 = (1.0_r_def + 1.0_r_def/sqrt2)*(pnonl + 1.0_r_def/sqrt2 +          &
                                  sqrt(pnonl*(sqrt2 - 1.0_r_def) + 0.5_r_def) )
        e2 = (1.0_r_def + 1.0_r_def/sqrt2)*(pnonl + 1.0_r_def/sqrt2 -          &
                                  sqrt(pnonl*(sqrt2 - 1.0_r_def) + 0.5_r_def) )
        gamma1 = i1
        gamma2 = i1 - e1
        ...
        call matrix_sweep(du_bl, du_1, cq_cm, cq_cm_1, df, gamma1, gamma2,     &
                          height_w1, dtr_rhodz, tau_loc, du_nt, rhokm, rdz,    &
                          k_blend, ndf_w2, undf_w2, map_w2, ndf_w1, undf_w1,   &
                          map_w1)
        ...
        if (fland > 0.0_r_def) then
          tau_land_star = ( gamma2 * tau_land_loc +                            &
                            gamma1 * rhokm(map_w2(df)) * du_1 ) /              &
                          (1.0_r_def + gamma1 * rhokm(map_w2(df)) * cq_cm_1)
        else
          tau_land_star = 0.0_r_def
        end if
        if (fland < 1.0_r_def) then
          tau_ssi_star = ( gamma2 * tau_ssi_loc +                              &
                           gamma1 * rhokm(map_w2(df)) * du_1 ) /               &
                         (1.0_r_def + gamma1 * rhokm(map_w2(df)) * cq_cm_1 )
        else
          tau_ssi_star = 0.0_r_def
        end if
        tau_star(0) = fland * tau_land_star + (1.0_r_def - fland) * tau_ssi_star

        !================================================================
        ! In the UM this happens in bdy_impl4 - predictor section
        !================================================================
        du_bl(map_w2(df)) = du_bl(map_w2(df)) - cq_cm(0) * tau_star(0)

        do k = 1, bl_levels-1
          du_bl(map_w2(df) + k) = du_bl(map_w2(df) + k) -                      &
                                  cq_cm(k) * du_bl(map_w2(df) + k-1)
          tau_star(k) = gamma2 * tau_loc(k) +                                  &
                        gamma1 * rhokm(map_w2(df) + k) * rdz(map_w1(df) + k) * &
                        ( du_bl(map_w2(df) + k) - du_bl(map_w2(df) + k-1) )
        end do

        do k = 0, bl_levels-1
          du_star(k) = du_bl(map_w2(df) + k)
        end do
        ...
        do k = 1, bl_levels-1
          tau_loc(k) = tau_loc(k) + rhokm(map_w2(df) + k) *                    &
                           ( du_bl(map_w2(df) + k) - du_bl(map_w2(df) + k-1) ) &
                           * rdz(map_w1(df) + k)
        end do
        ...
        if (fland > 0.0_r_def) then
          tau_land_loc = ( gamma2 * ( tau_land_loc +                           &
                                      rhokm(map_w2(df)) * du_star(0) )         &
                                    + gamma1 * rhokm(map_w2(df)) * du_1 ) /    &
                         (1.0_r_def + gamma1 * rhokm(map_w2(df)) * cq_cm_1 )
        else
          tau_land_loc = 0.0_r_def
        end if
        if (fland < 1.0_r_def) then
          ...
          tau_ssi_loc = 0.0_r_def
        end if
        tau_loc(0) = fland * tau_land_loc + (1.0_r_def - fland) * tau_ssi_loc

        !================================================================
        ! In the UM this happens in bdy_impl4 - corrector section
        !================================================================
        du_bl(map_w2(df)) = du_bl(map_w2(df)) - cq_cm(0) * tau_loc(0)
        ...
        do k = 0, bl_levels-1
          du_bl(map_w2(df) + k) = du_bl(map_w2(df) + k) + du_star(k)
        end do
        ...
        if (fric_heating) then
          do k = 0, bl_levels-2
            dissip(map_w2(df) + k) = tau_loc(k+1) * rdz(map_w1(df) + k+1) *    &
                     ( u_physics(map_w2(df) + k+1) + du_bl(map_w2(df) + k+1) - &
                         u_physics(map_w2(df) + k) - du_bl(map_w2(df) + k) )
          end do
          ...
          dissip(map_w2(df)+bl_levels-1) = 0.0_r_def
        end if
  ...
  subroutine matrix_sweep(du_bl, du_1, cq_cm, cq_cm_1, df, gamma1, gamma2,     &
                          height_w1, dtrdz, tau, du_nt, rhokm, rdz, k_blend,   &
                          ndf_w2, undf_w2, map_w2, ndf_w1, undf_w1, map_w1)
    ...
    real(kind=r_def), dimension(undf_w1), intent(in) :: height_w1, rdz
    real(kind=r_def), dimension(undf_w2),  intent(in) :: rhokm
    real(kind=r_def), dimension(0:bl_levels-1), intent(in) :: du_nt, tau, dtrdz

    integer(kind=i_def), intent(in) :: df, k_blend
    real(kind=r_def), intent(in) :: gamma1, gamma2

    ! Passed out
    real(kind=r_def), dimension(undf_w2),  intent(inout) :: du_bl
    real(kind=r_def), dimension(0:bl_levels-1), intent(out) :: cq_cm
    real(kind=r_def), intent(out) :: du_1, cq_cm_1

    ! Internal variables
    integer(kind=i_def) :: k
    real(kind=r_def) :: r_sq, rr_sq, rbm, am, cu_prod

    ! Topmost level
    k = bl_levels-1
    r_sq = (height_w1(map_w1(df) + k) + radius)**2
    du_bl(map_w2(df) + k) = -dtrdz(k) * r_sq * tau(k)
    ! addition of non-turbulent increments
    du_bl(map_w2(df) + k) = gamma2 * ( du_bl(map_w2(df) + k) + du_nt(k) )

    cq_cm(k) = -dtrdz(k) * gamma1 * r_sq * rhokm(map_w2(df) + k)               &
               * rdz(map_w1(df) + k)

    rbm = 1.0_r_def / (1.0_r_def - cq_cm(k) )
    du_bl(map_w2(df) + k) = rbm * du_bl(map_w2(df) + k)
    cq_cm(k) = rbm * cq_cm(k)
    ...
    do k = bl_levels-2,1,-1
      ...
      du_bl(map_w2(df) + k) = dtrdz(k) * ( rr_sq * tau(k+1) - r_sq * tau(k) )
      ! addition of non-turbulent increments
      du_bl(map_w2(df) + k) = gamma2 * ( du_bl(map_w2(df) + k) + du_nt(k) )

      am = -dtrdz(k) * gamma1 * rr_sq * rhokm(map_w2(df) + k+1)                &
           * rdz(map_w1(df) + k+1)

      cq_cm(k) = -dtrdz(k) * gamma1 * r_sq * rhokm(map_w2(df) + k)             &
                 * rdz(map_w1(df) + k)

      rbm = 1.0_r_def / (1.0_r_def - cq_cm(k) - am * (1.0_r_def + cq_cm(k+1) ) )

      du_bl(map_w2(df) + k) = du_bl(map_w2(df) + k) -                          &
                              am * du_bl(map_w2(df) + k+1)
    ...
    du_bl(map_w2(df) + k) = dtrdz(k) * rr_sq * tau(k+1)
    ! addition of non-turbulent increments
    du_bl(map_w2(df) + k) = gamma2 * ( du_bl(map_w2(df) + k) + du_nt(k) )
    ...
    rbm = 1.0_r_def / (1.0_r_def - am * (1.0_r_def + cq_cm(k+1) ) )

    du_bl(map_w2(df) + k) = du_bl(map_w2(df) + k) - am * du_bl(map_w2(df) + k+1)
    ...
    k = k_blend
    du_1 = du_bl(map_w2(df) + k)
    cu_prod = cq_cm(k)
    do k = k_blend-1,0,-1
      du_1 = du_1 + ( (-1) ** k_blend ) * du_bl(map_w2(df) + k) * cu_prod
